...
* **Details:** 1. Remove `world["map"]` from the broadcast state. 
  ...
  3. Modify physics: when a bullet hits a destructible wall (`2`), change the grid value to floor (`0`), and inject a `{"event": "WALL_DESTROYED", "r": Y, "c": X}` delta in events object into the next tick.
...
This will be a considerable overhaul in how world is emmited.  